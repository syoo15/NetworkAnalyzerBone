...
<<odplot>>=
ggplot(subset(growth_data, dilution>0.0002), aes(x=dilution, y=Mean)) + geom_point() + scale_y_log10() + scale_x_log10()
ggplot(subset(growth_data, dilution>0.0002), aes(x=dilution, y=as.numeric(Resistance))) + geom_point() + scale_x_log10()
@
